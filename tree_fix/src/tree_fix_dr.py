...
def callback(trees_scan):
    ...
    for p in scan_pts_in_world_xy:
        ...
        for (i, sp) in enumerate(static_pts):
            ...
            if dist < mindist and not used[i]:  
                found = True
                if bestptidx is not None:
                    used[bestptidx] = False
                bestpoint = sp
                bestptidx = i
                mindist = dist
                used[i] = True
    ...
    last_pos = numpy.array([(last_pos[0]+new_pos[0])/2,
    ...
    if dtime == 0:
        ...
        vyaw = (new_yaw - last_yaw)/dtime * 0
        vpos = (new_pos - last_pos)/dtime * 0
    #print(last_yaw, new_yaw, vyaw)
    last_time = rospy.Time.now()
    tf_publisher.sendTransform(last_pos,
                               quaternion_from_euler(0,0,last_yaw),
                               rospy.Time.now(),
